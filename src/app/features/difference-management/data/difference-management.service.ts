...
export class DifferenceManagementService {
  ...
  // No hay backend todavía — esto solo arma el payload que se enviaría. El
  // componente decide qué hacer con el resultado (toast + navegación).
  save(): SaveResult | null {
    const order = this.order();
    if (!order || !this.canSave()) return null;
